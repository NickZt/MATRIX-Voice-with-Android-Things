      ...
      //  irq_cv.wait(lock_);
        ...
        if (!bus_->Read(kMicrophoneArrayBaseAddress,
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "TODEL ",
                                "mics.Read(); return false;");
            return false;
        }
        __android_log_print(ANDROID_LOG_DEBUG, "TODEL ",
                            "mics.Read(); irq_cv.wait(lock_); OK");
...
// Setting fifos for the 'delay & sum' algorithm
    void MicrophoneArray::CalculateDelays(float azimutal_angle, float polar_angle,
        ...
        if (sound_speed_mmseg == 0) {
            __android_log_print(ANDROID_LOG_ERROR, "TODEL ",
                                "Bad Configuration, Sound Speed must be greather than 0");
    ...
    bool MicrophoneArray::SetSamplingRate(uint32_t sampling_frequency) {
        if (sampling_frequency == 0) {
            __android_log_print(ANDROID_LOG_ERROR, "TODEL ",
                                "Bad Configuration, sampling frequency must be greather than 0");
        ...
        for (int i = 0;; i++) {
            if (MIC_sampling_frequencies[i][0] == 0) {
                __android_log_print(ANDROID_LOG_ERROR, "TODEL ",
                                    "Unsoported sampling frequency, it must be: 8000, 12000,16000, 22050, 24000, 32000, 44100, 48000, 96000");
