...
namespace matrix_hal {
    ...
    bool MicrophoneCore::SetCustomFIRCoeff(
        ...
        if (custom_fir.size() == kNumberFIRTaps) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, "MicrophoneCore",
                                "Size FIR Filter must be : %d", kNumberFIRTaps);
            return false;
        }
    }
    ...
    bool MicrophoneCore::SelectFIRCoeff(FIRCoeff *FIR_coeff) {
        ...
        if (sampling_frequency == 0) {
            __android_log_print(ANDROID_LOG_ERROR, "MicrophoneCore", "Bad Configuration, sampling_frequency must be greather than 0");
            return false;
        }
        ...
        for (int i = 0;; i++) {
            if (FIR_coeff[i].rate_ == 0) {
                __android_log_print(ANDROID_LOG_ERROR, "MicrophoneCore",
                                    "Unsoported sampling frequency, it must be: 8000, 12000, "
            ...
            if (FIR_coeff[i].rate_ == sampling_frequency) {
                if (FIR_coeff[i].coeff_.size() == kNumberFIRTaps) {
                    ...
                    return SetFIRCoeff();
                            ...
                            __android_log_print(ANDROID_LOG_ERROR, "MicrophoneCore",
                        "Size FIR Filter must be:-> %d --- %d ",
                                                kNumberFIRTaps,
                                                FIR_coeff[i].coeff_.size());
                }
            }
        }
    }
